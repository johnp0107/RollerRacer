...
void URaceTrackingWidget::SetLapText(FString LapText)
{
	if (LapTextBlock)
	{
		...
		LapTextBlock->SetText(FText::FromString("Lap: " + LapText));
...
void URaceTrackingWidget::SetPositionImages(TArray<UTexture2D*> Textures)
{
	if (FirstPlaceImage)
	{
		...
		FirstPositionText->SetVisibility(ESlateVisibility::Visible);
	}
	if (Textures.Num() > 1)
	{
		if (SecondPlaceImage)
		{
			...
			SecondPositionText->SetVisibility(ESlateVisibility::Visible);
		}
	}
	if (Textures.Num() > 2)
	{
		if (ThirdPlaceImage)
		{
			...
			ThirdPositionText->SetVisibility(ESlateVisibility::Visible);
		}
	}
	if (Textures.Num() > 3)
	{
		if (ForthPlaceImage)
		{
			...
			ForthPositionText->SetVisibility(ESlateVisibility::Visible);
		}
	}
}